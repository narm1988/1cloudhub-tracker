...
export const SECTION_HEADER = 'text-[13px] font-medium text-gray-900'
...
export function InlineDescription({ value, onSave }: { value: string; onSave: (v: string) => void }) {
  ...
  if (!editing) {
    return (
      <div
        onClick={() => setEditing(true)}
        className="text-[13px] text-gray-700 leading-relaxed whitespace-pre-wrap rounded-md px-1.5 -mx-1.5 py-1 hover:bg-gray-50 transition-colors cursor-text min-h-[32px]"
      >
  ...
  return (
    <textarea
      ...
      className="w-full px-2.5 py-2 rounded-lg border border-brand text-[13px] outline-none focus:ring-1 focus:ring-brand/30 resize-y min-h-[100px]"
    />
  )
}
...
function StatusSelect({ status, onChange }: { status: Status; onChange: (s: Status) => void }) {
  ...
  return (
    <select
      ...
      aria-label="Status"
      className={`text-[11px] font-semibold pl-2.5 pr-6 py-0.5 rounded-md whitespace-nowrap outline-none cursor-pointer border-0 appearance-none focus:ring-2 focus:ring-brand/50 ${meta.tailwind}`}
      style={{
      ...
      {isTerminal && (
        <div className="relative">
          <button
            ...
            className="flex items-center gap-1 text-[12px] font-semibold text-brand hover:text-brand-deep px-2 py-0.5 rounded-md border border-brand/30 hover:bg-brand-soft transition-colors"
          >
              ...
              <div className="absolute left-0 top-full mt-1 w-36 bg-white border border-gray-200 rounded-lg shadow-lg z-50 py-1">
                {REOPEN_TARGETS.map((s) => (
                  <button
                    ...
                    className="w-full text-left px-3 py-1.5 text-[13px] text-gray-900 hover:bg-gray-50"
                  >
                    {s}
                  </button>
                ))}
  ...
  return (
    <div className="relative">
      ...
      >
        {assignee ? (
          <>
            <span className="text-[13px] text-gray-900 font-medium">{assignee.full_name}</span>
          ...
          <div className="absolute right-0 top-full mt-1 w-56 bg-white border border-gray-200 rounded-lg shadow-lg z-50 py-1 max-h-64 overflow-y-auto">
            {currentUserId && assigneeId !== currentUserId && (
              <button
                ...
                className="w-full text-left px-3 py-1.5 text-[13px] text-brand hover:bg-gray-50 font-medium"
              >
                Assign to me
              </button>
            )}
            <button
              ...
              className="w-full text-left px-3 py-1.5 text-[13px] text-gray-500 hover:bg-gray-50"
            >
              Unassigned
            </button>
...
export function CommentInput({ onSubmit }: { onSubmit: (content: string) => void }) {
  ...
  return (
    <div className="flex gap-3">
      <textarea
        className="flex-1 px-3 py-2 rounded-md border border-gray-200 text-[13px] shadow-sm outline-none focus:border-brand focus:ring-2 focus:ring-brand/10 resize-none min-h-[60px]"
