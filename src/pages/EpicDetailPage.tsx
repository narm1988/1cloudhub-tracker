  ...
  return (
    <div>
      ...
      <button
        ...
        className="flex items-center gap-1.5 text-gray-500 text-body hover:text-gray-700 transition-colors mb-4"
      >
      ...
      <div className="flex items-start justify-between mb-6">
        <div>
          <span className="font-mono text-label text-gray-400">{epic.id.slice(0, 8)}</span>
          <h1 className="font-display text-heading font-semibold text-ink mt-0.5">{epic.title}</h1>
          {epic.description && (
            <p className="text-body text-gray-500 mt-1.5 max-w-xl">{epic.description}</p>
          )}
          <div className="flex items-center gap-4 mt-2 text-body text-gray-500">
          ...
          return (
            <div key={status} className="w-[240px] shrink-0">
              <div className="flex items-center gap-2 mb-3">
                <span className="text-label font-semibold text-gray-500 uppercase tracking-wide">
                  {status}
                </span>
                <span className="text-caption font-mono tabular-nums text-gray-400 bg-gray-100 rounded-full px-2 py-0.5">
                  {items.length}
                </span>
              </div>
              ...
              <div className="space-y-2.5">
                ...
                {items.length === 0 && (
                  <div className="border border-dashed border-gray-200 rounded-lg py-4 text-center text-label text-gray-400">
                    No stories
                  </div>
                )}
              </div>
...
function StoryCard({ story, onClick }: { story: Story; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <div className="text-body text-ink font-medium leading-snug mb-2.5">
        {story.title}
      </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-1.5">
          {story.assignee ? (
            ...
            <span className="text-caption text-gray-400">Unassigned</span>
          )}
        </div>
