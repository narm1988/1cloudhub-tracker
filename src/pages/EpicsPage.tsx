...
export default function EpicsPage() {
  ...
  async function createEpic(title: string, description: string) {
    const { error } = await supabase.from('epics').insert({
      title,
      description,
      owner_id: user?.id,
      project_id: null, // Will be linked to project later
      status: 'Created',
    })
    ...
    <div>
      ...
      {epics.length === 0 ? (
        <div className="text-center py-20 text-gray-400">
          ...
          <p className="text-sm mt-1">Create your first epic to get started.</p>
        </div>
      ) : (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
          {epics.map((epic) => (
            ...
            >
              ...
              <div className="text-[14.5px] font-semibold text-ink mb-1.5">{epic.title}</div>
